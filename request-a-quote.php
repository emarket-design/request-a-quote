...
if (!class_exists('Request_a_quote')):
	...
	final class Request_a_quote {
		...
		private function define_constants() {
			define('REQUEST_A_QUOTE_VERSION', '2.3.3');
		...
		private function includes() {
			...
			if (!function_exists('emd_get_country_list')) {
				require_once REQUEST_A_QUOTE_PLUGIN_DIR . 'includes/country-functions.php';
			}
			...
			if (is_admin()) {
				...
				if (!class_exists('Emd_List_Table')) {
					require_once REQUEST_A_QUOTE_PLUGIN_DIR . 'includes/admin/class-emd-list-table.php';
				}
				if (!function_exists('emd_show_shortcodes_page')) {
					require_once REQUEST_A_QUOTE_PLUGIN_DIR . 'includes/admin/shortcode-list-functions.php';
				}
				//these files are in all apps
			...
			if (!function_exists('emd_show_forms_lite_page')) {
				require_once REQUEST_A_QUOTE_PLUGIN_DIR . 'includes/emd-form-builder-lite/emd-form-builder.php';
			}
			if (!function_exists('emd_lite_modal')) {
				require_once REQUEST_A_QUOTE_PLUGIN_DIR . 'includes/emd-lite/emd-lite.php';
			}
		...
		public function display_settings() {
			...
			add_menu_page(__('Quotes Settings', $this->textdomain) , __('Quotes Settings', $this->textdomain) , $settings_pages_cap, $this->app_name, array(
				$this,
