...
$container = apply_filters('emd_change_container','emd-wrapper','request_a_quote', 'archive');  
$has_sidebar = apply_filters( 'emd_show_temp_sidebar', 'right', 'request_a_quote', 'archive');
...
<?php
	do_action('emd_archive_before_header', 'request-a-quote',$post->post_type);
	emd_get_template_part('request-a-quote', 'archive', str_replace("_","-",$post->post_type) . '-aheader');
	do_action('emd_archive_after_header', 'request-a-quote',$post->post_type);
?>
	</div>
...
<?php
	while ( have_posts() ) : the_post(); ?>
			...
			<?php emd_get_template_part('request-a-quote', 'archive', str_replace("_","-",$post->post_type)); ?>
			</div>
...
<?php
	do_action('emd_archive_before_footer', 'request-a-quote',$post->post_type);
	emd_get_template_part('request-a-quote', 'archive', str_replace("_","-",$post->post_type) . '-afooter');
	do_action('emd_archive_after_footer', 'request-a-quote',$post->post_type);
?>
</div>
<?php	$has_navigation = apply_filters( 'emd_show_temp_navigation', true, 'request_a_quote', 'archive');
	if($has_navigation){
		global $wp_query;
