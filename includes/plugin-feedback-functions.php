...
function request_a_quote_show_optin() {
	...
	if (!get_option('request_a_quote_tracking_optin')) {
		...
		echo $tr_msg . '</p>';
		echo '<p>' . __('Please confirm your email address below to start receiving emails from us.', 'request-a-quote') . '</p>';
		$current_user = wp_get_current_user();
		...
		echo '<li class="emd-permission"><i class="dashicons dashicons-email-alt"></i><div><span>' . __('Newsletter', 'request-a-quote') . '</span><p>' . __('Updates, announcements, marketing, no spam', 'request-a-quote') . ', <a href="https://emdplugins.smartlamb.com/subscription-preferences/" target="_blank">unsubscribe anytime</a></p></div></li>';
